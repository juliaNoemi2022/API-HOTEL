...
const validarExisteHabitacionPorNumero =  async (req, res, next) => {
...
const validarNoExisteHabitacionPorNumero =  async (req, res, next) => {
    const numero = req.body.numero;
    const habi = await Habitaciones.findOne({where: {numero}, order: [
        ['numero', 'ASC']]})
    if(!habi){
...
module.exports = { existeHabitacionPorId3,existeHabitacionPorId2,validarNoExisteHabitacionPorNumero,validarExisteHabitacionPorNumero, existeHabitacionPorId, existeHabitacionPorNumero, controlCapacidadHabitacion}